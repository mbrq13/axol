...
- The `lerobot` and `video` extras installed on the main host. The [quick install](/installation#quick-install-recommended) includes both; on a development install run `uv sync --extra lerobot --extra video` (see [Installation](/installation)).
...
<Steps>
  ...
  <Step title="Stream the cameras">
    ...
    Streams from the NIC's current IP until `Ctrl+C`. Note that IP — you'll pass it to the main host as `--robot_config.zed_host`. If the overhead is a stereo ZED X, add `--overhead-stereo` (and set `--robot_config.cameras.overhead.stereo true` on the main host — the dataset then records `overhead_left` / `overhead_right`). See [`zed.stream`](/cli/zed-stream) for resolution, fps, and bitrate flags.
  </Step>
</Steps>
...
<Note>
  The camera feeds are relayed to the headset during collection (requires the `video` extra on the main host), so you can line up takes from the robot's point of view.
</Note>
