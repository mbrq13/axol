...
When the host has camera video enabled ([`axol teleop --zed_host`](/cli/teleop), [`collect-data`](/cli/collect-data), or the [control panel](/guides/control-panel) with a connected ZED box), the robot's camera feeds stream to the headset over WebRTC and render as screens over passthrough.
